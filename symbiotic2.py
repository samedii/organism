...
def apply_organism(x, organisms):
...
def evolve(problem_organisms, evolve_organisms):
    # only single scale first
    flat_problem_organisms = flatten_organisms(problem_organisms)

    evolve_scale = softplus(evolve_organisms)
    child_problem_organisms = np.random.normal(loc=flat_problem_organisms, scale=evolve_scale.reshape(-1, 1), size=flat_problem_organisms.shape)
    child_evolve_organisms = np.random.normal(loc=evolve_organisms, scale=evolve_scale, size=evolve_organisms.shape)
...
def predict(X, problem_organisms):
    return apply_organism(X, problem_organisms)
...
def main():
    ...
    evolve_organisms = np.random.normal(loc=0, scale=0.01, size=(n_organisms, 1, 1, 1))
    ...
    for iteration in range(n_iterations):
        ...
        evolve_organisms = evolve_organisms[organism_rank][:n_survivals]
        ...
        evolve_organisms = np.concatenate((evolve_organisms, child_evolve_organisms), axis=0)
    ...
    best_evolve_organisms = evolve_organisms[organism_rank][:n_survivals]
