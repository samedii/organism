...
def init_state(n_organisms, n_problems):
...
def predict(xs, read_nn, write_nn, n_organisms, n_problems):
    state = init_state(n_organisms, n_problems)
...
def evolve_layer(state, layer, read_nn, write_nn):
    layer_weight, layer_bias = layer

    scale = softplus(state_to_output(state, write_nn))
    child_layer_weight = np.random.normal(loc=layer_weight, scale=scale, size=layer_weight.shape)
    child_layer_bias = np.random.normal(loc=layer_bias, scale=scale, size=layer_bias.shape)

    return child_layer_weight, child_layer_bias

def evolve_nn(state, nn, read_nn, write_nn):
    n_organisms = read_nn[0][0].shape[0]
    ...
    for layer_index, layer in enumerate(nn):
        ...
        child_nn.append(evolve_layer(state, layer, read_nn, write_nn))
    return child_nn
...
def evolve(read_nn, write_nn):
    ...
    state = init_state(n_organisms, n_problems=1)
    state = input_to_state(-1*one, state, read_nn)
    state = input_to_state(-1*one, state, read_nn)
    state = input_to_state(-1*one, state, read_nn)
    child_read_nn = evolve_nn(state, read_nn, read_nn, write_nn)

    state = init_state(n_organisms, n_problems=1)
    state = input_to_state(1*one, state, read_nn)
    state = input_to_state(1*one, state, read_nn)
    state = input_to_state(1*one, state, read_nn)
    child_write_nn = evolve_nn(state, write_nn, read_nn, write_nn)
...
def init_nns():
    ...
    write_nn = (
        ...
        [init_layer(n_organisms, write_layer_size, 1)]
    )
